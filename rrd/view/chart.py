...
from rrd.utils.rrdgraph import p95
...
def chart_before():
    if request.method == "GET":
        ...
        g.p95 = request.args.get("p95") or "off" #是否求和
    ...
    if g.p95 == "on":
        ...
        for serie in series:
            ...
            if len(serie_vs) > max_size:
                max_size = len(serie_vs)
                tmp_ts = [x[0] for x in serie["data"]]
            max_p95 = p95(serie_vs)
        max_p95_data = []
        for i in range(0,max_size):
            max_p95_data.append((tmp_ts[i],max_p95))
        p95_serie['data'] = max_p95_data
        series.append(p95_serie)
